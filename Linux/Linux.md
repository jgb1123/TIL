...
환경설정을 통해 컴퓨터의 세팅을 설정하듯, 환경변수 설정을 통해 프로그램이 동작하는 환경을 설정할 수 있다.

환경변수는 지역 환경변수와 전역 환경변수로 분류된다.
> 지역 환경변수
> * 해당 터미널에서만 사용할 수 있는 환경변수.
>
> 전역 환경변수
> * 다른 터미널에서도 사용할 수 있는 환경변수
...
`환경변수명=값` 과 같이 입력하면 된다.


#### b) 전역 환경변수


`export 환견변수명 = 값` 과 같이 입력하면 된다.

### 2) 환경변수 영구 적용
편집기로 파일을 수정 후 `source` 명령어를 이용하면 된다.
#### a) 지역 환경변수
...
`$HOME/.bash_profile` 쉘 함수, Alias 시스템 전역 변수 정의

### 3) 환경변수 확인
* 특정 환경변수의 값을 확인할 때에는 `echo` 명령어를 쓰면 된다.
* `export`명령어를 이용하면 전체 환경변수 목록을 확인 할 수 있다.
