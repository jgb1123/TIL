...
## 애드온(Add-on)
* 애드온은 쿠버네티스 리소스(DemonSet, Deployment 등)을 이용하여 클러스터 기능을 구현한다.
* 클러스터 단위의 기능을 제공하기 때문에 애드온은 데핸 네임스페이스 리소스는 kube-system 네임스페이스에 속한다.
* 애드온이 사용되는 Pod는 Deployment, Replication controller 등에 의해 관리된다.

### DNS 애드온
* 클러스터 내의 서비스 디스커버리 및 통신을 위한 DNS를 제공한다.
* 클러스터 내에서 서비스에 DNS 이름을 사용하여 액세스할 수 있도록 도와준다.
* 예시로 CoreDNS가 있다.

### 대시보드 애드온
* 클러스터를 시각화하고 관리하기 위한 웹 기반 UI를 제공한다.
* 대시보드를 통해 클러스터의 현황을 쉽게 모니터링하고 제어할 수 있다.
* 예시로 Kubernetes Dashboard가 있다.

### 컨테이너 리소스 모니터링 애드온
* 클러스터 내의 컨테이너 리소스 사용량을 수집하고 모니터링하는 도구를 제공한다.
* 이를 통해 성능 관련 이슈를 파악하고 최적화할 수 있다.
* 예시로 Metrics Server가 있고, HPA와 통합하여 사용된다.

### 클러스터-레벨 로깅 애드온
* 클러스터 내의 컨테이너 로그를 수집하고 중앙 로그 저장소에 저장하는 메커니즘을 제공한다.
* 이를통해 로그 데이터를 효과적으로 관리하고 분석할 수 있다.
* 예시로 ELK 스택, EFK 스택이 있다.

### 인그레스 컨트롤러 애드온
* 외부에서 클러스터 내의 서비스로 트래픽을 라우팅하는 역할을 담당한다.
* 예시로 NGINX Ingress Controller가 있다.

### 스토리지 애드온
* 클러스터 내에서 데이터를 저장하고 관리하기 위한 스토리지 솔루션을 제공한다.
* 이를통해 데이터를 안전하게 보관하고 관리할 수 있다.
* 예시로 Rook이 있다.

### 설치 및 업그레이드 애드온
* 클러스터 자체나 다른 애드온을 설치하고 업그레이드하기 위한 도구를 제공한다.
* 예시로 kubeadm이 있다.

### 매니페스트 및 커스텀 리소스 애드온
* 쿠버네티스 오브젝트를 생성하고 관리하는데 도움을 주는 도구들이다.
* 예시로 Helm이 있다.
