...
## Timer(대기시간 설정)
* 테스트 시나리오를 작성할 때 실제 동작과 유사하게 모델링을 해야 테스트의 정확도를 높힐 수 있다.
* JMeter에서 동작 사이에 대기시간을 타이머를 통해 설정할 수 있다.
* 쓰레드 그룹에서 Add - Timer - Constant Timer를 선택하면 타이머 설정을 할 수 있으며, 이 화면에서 대기에 필요한 시간을 입력하면 실제 테스트시 해당 시간만큼 대기했다가 다음 작업을 진행한다.

## Listener(테스트 상태 수집)
* 리스너는 테스트한 결과를 수집하는 역할을 하며, 리스너를 통해 모든 결과를 분석하고 테스트한 애플리케이션의 성능을 확인할 수 있다.
* 결과 서버에서 응답한 정보를 토대로 결과 정보를 보여주거나 저장하는 역할을 하며, 사전에 각 리스너에 대한 기능을 잘 이해하고 작성한 시나리오에 합당한 리스너를 추가해야 한다.
* 테스트 시 리스너에서 보여주는 결과를 해결할 수 있는 능력 또한 필요하다.

## Variable(변수 설정)
* JMeter에서 변수를 사용하는 방법을 알면 테스트 플랜을 작성하거나 실제 테스트를 수행할 때 유용하게 사용할 수 있다.
  * 샘플러를 이용한 테스트에서 변수를 사용하지 않게 되면 POST 혹은 GET으로 전달한 파라미터 값을 하나밖에 사용할 수 없다.
  * 실제로는 대부분 미들웨어나 데이터베이스가 같은 값으로 조회하면 캐싱기능을 통해 매우 빠르게 응답하기 때문에, 이는 성능테스트에 매우 큰 문제를 발생시킨다.
  * 따라서 테스트 수행 시 적절하게 파라미터 값을 변경해가며 테스트를 하는것이 일반적이며, 이러한 요구를 수용하기 위해 JMeter는 CSV파일을 이용해 여러개의 파라미터 데이터를 정의해 사용할 수 있다.
* JMeter에서 변수명에 대한 값을 참조할 때는 `${변수명}` 형태로 사용한다.
  * 변수명은 CSV Data Set Config에서 지정한 변수명과 일치해야 한다.
  * JMeter 테스트 수행 시 쓰레드가 CSV파일의 데이터를 한줄 씩 읽어가면서 테스트를 수행한다.
* 변경의 소지가 높은 값들을 추가로 변수처리해서 테스트 플랜을 작성하면 유용하다.
  * 대표적인 것이 서버 주소와 포트 정보이며, 아무리 Config Elements의 HTTP Request Defaults를 이용해 정의하더라도 이 역시 해당 설정이 많으면 변경해야 하는 경우가 많다.
  * 테스트 플랜에서 변수를 지정한 후 해당 값을 사용하면 변수에 설정된 값으로 치환해 테스트가 실행된다.
* Add - Config Element - User Defined Variables 메뉴에서도 정의할 수 있고, Random Variables와 같은 특정 구간의 숫자 범위에 해당하는 값을 랜덤하게 취해서 사용하는 변수를 정의할 수도 있다.
...
### 원격 테스트
* 테스트시, 가급적이면 원격으로 운영되고 있는 서버와 네트워크 내에서 테스트하는 것이 좋다.
