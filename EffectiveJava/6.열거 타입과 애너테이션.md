...
* 실제로 Enum API문서에도 ordinal에 대부분 프로그래머는 이 메서드를 쓸 일이 없다고 되어 있다.
* ordinal은 `EnumSet`과 `EnumMap` 같이 열거 타입 기반의 범용 자료구조에 쓸 목적으로 설계되었기 때문에, 이런 용도가 아니라면 ordinal 메서드는 절대 사용하면 안된다.
