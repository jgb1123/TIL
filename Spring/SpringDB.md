...
#### Spring의 트랜잭션 리소스 동기화
* 트랜잭션을 유지하려면 트랜잭션의 시작부터 끝까지 같은 데이터베이스의 커넥션을 유지해야 한다.
* 파라미터로 커넥션을 전달하는 방법이 있지만, 코드가 지저분해진다.(커넥션에 따라 메서드도 중복해서 만들어야 함)
* 그래서 스프링은 **트랜잭션 동기화 매니저를 제공**한다.
* 트랜잭션 동기화 매니저는 쓰레드 로컬을 사용해 커넥션을 동기화 해주고, 트랜잭션 매니저는 내부에서 이 트랜잭션 동기화 매니저를 사용한다.
  * 트랜잭션 동기화 매니저는 쓰레드 로컬을 사용하기 때문에 멀티쓰레드 상황에서도 안전하게 커넥션을 동기화 할 수 있다.
* 커넥션이 필요하면 트랜잭션 동기화 매니저를 통해 커넥션을 획득하면 된다. (파라미터로 커넥션을 전달받으면서 사용하지 않아도 됨)
* 트랜잭션 동기화 매니저가 관리하는 커넥션이 없는 경우 새로운 커넥션을 생성해서 반환한다. (트랜잭션이 있어야 하는 경우와 없어야 하는 경우 둘 다 처리가능)
##### 트랜잭션 동기화 매니저 동작 방식
1. 트랜잭션 매니저는 데이터소스를 통해 커넥션을 만들고 트랜잭션을 시작한다.
2. 트랜잭션 매니저는 트랜잭션이 시작된 커넥션을 트랜잭션 동기화 매니저에 보관한다
3. 리포지토리는 트랜잭션 동기화 매니저에 보관된 커넥션을 꺼내서 사용한다. (파라미터로 커넥션 전달하지 않아도 됨)
    * 트랜잭션 동기화 매니저가 관리하는 커넥션이 없는 경우 새로운 커넥션을 생성해서 반환한다. (트랜잭션이 있어야 하는 경우 없어야 하는 경우 둘다 처리 가능)
4. 트랜잭션이 종료되면 트랜잭션 매니저는 트랜잭션 동기화 매니저에 보관된 커넥션을 통해 트랜잭션을 종료하고 커넥션을 닫는다.

### 트랜잭션 템플릿
* 트랜잭션을 시작하려면 try, catch를 포함한 성공시 커밋, 실패시 롤백 코드가 반복되어야 한다.
* 템플릿 콜백 패턴을 활용하면 이러한 문제를 해결할 수 있다.
* 스프링은 `TransactionTemplate`이라는 트랜잭션 템플릿을 제공한다.
* `execute()` : 응답값이 있을 때 사용
* `executeWithoutResult()` : 응답 값이 없을 때 사용
* 트랜잭션 템플릿을 사용하려면 르랜잭션 매니저가 필요하기 때문에, 생성자에서 트랜잭션 매니저를 주입받으면서 트랜잭션 템플릿을 생성하면 된다.
* 트랜잭션 템플릿으로 트랜잭션을 사용할 때 반복하는 코드를 제거할 수 있다.
* 하지만 트랜잭션 템플릿만으로는 서비스 계층에 순수한 비즈니스 로직만 남길 순 없다.
* 이러한 문제를 해결하기 위해 스프링 AOP를 통해 프록시를 도입하면 된다.

### 스프링이 제공하는 트랜잭션 AOP
* 프록시를 도입하기 전에는 서비스의 로직에서 트랜잭션을 직접 시작한다.
* 프록시를 사용하면 트랜잭션을 처리하는 객체와 비즈니스 로직을 처리하는 서비스 객체를 명확하게 분리할 수 있다.
* 스프링이 제공하는 AOP기능을 사용하면 프록시를 매우 편리하게 적용할 수 있다.
* **트랜잭션 처리가 필요한 곳에 `@Transactional` 애너테이션만 붙여주면 된다.** (메서드단, 클래스단에 모두 붙일 수 있음)
* 스프링의 트랜잭션 AOP는 이 애너테이션을 인식해서 트랜잭션 프록시를 적용해준다.
* 따라서 트랜잭션 관련 코드는 사라지고, 비즈니스 로직만 남게 된다.
* 참고로 `AopUtils.isAopProxy()` 메서드를 이용하면 Aop 프록시인지 확인할 수 있다.


___
참고

[인프런 김영한님의 Spring DB 1편](https://www.inflearn.com/course/%EC%8A%A4%ED%94%84%EB%A7%81-db-1/dashboard)