...
## Spring Container
### Spring Container?
* 스프링 컨테이너는 Spring의 핵심 컴포넌트이다.
* 내부에 존재하는 애플리케이션 Bean의 생명주기를 관리한다. (Bean 생성, 관리 제거 등)
* 스프링 컨테이너는 XML, 애너테이션 기반의 자바 설정클래스로 만들 수 있다.
* 의존성 주입을 통해 애플리케이션의 컴포넌트를 관리한다
  * 서로 다른 Bean을 연결해 애플리케이션의 Bean들을 연결하는 역할을 한다.
  * 모듈 간에 의존 및 결합으로 인해 발생하는 문제로부터 자유로울 수 있다.
* 일반적으로 `ApplicationContext`를 스프링 컨테이너라고 하며, 인터페이스로 구현되어 있다. (다형성)
  * BeanFactory를 직접 사용하는 경우가 거의 없기 때문에 보통 ApplicationContect를 스프링 컨테이너라고 한다.
#### Bean?
* 스프링 컨테이너가 관리하는 자바 객체를 의미하며, 하나 이상의 Bean을 관리한다.
* Bean은 클래스의 등록 정보, 게터/세터 메서드를 포함한다.
* 컨테이너에 사용되는 설정 메타데이터로 생성된다.
#### BeanDefinition
* `BeanDefinition`이라는 추상화로 인해 스프링이 다양한 설정 형식을 지원할 수 있다.
